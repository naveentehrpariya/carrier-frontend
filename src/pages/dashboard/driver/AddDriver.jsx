...
export default function AddDriver({fetchLists}){
    ...
    const add_driver = () => {
      setLoading(true);
      const resp = Api.post(`/driver/add`, data);
      resp.then((res) => {
        setLoading(false);
  ...
  return (
    <div>
      <Popup action={action} size="md:max-w-2xl" space='p-8' bg="bg-black" btnclasses="" btntext={"Add New Driver"} >
         ...
         <div className='flex justify-center items-center'>
            <button  onClick={add_driver} className="btn md mt-6 px-[50px] main-btn text-black font-bold">{loading ? "Logging in..." : "Submit"}</button>
         </div>
      </Popup>
    </div>
  )
