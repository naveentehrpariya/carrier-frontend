...
export default function ChargesItems() {
  ...
  return (
      <div className='mt-12'> 
         <div className='flex justify-between items-center mb-4 '>
            <h2 className='text-white text-2xl'>Revenue Charges</h2>
         </div>
         {loading ? <Loading /> :
         <div className='fgdg'>
             <ul className='grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3'>
                {lists.map((e) => (
                  <li key={e._id} className='text-white w-full border border-gray-800 my-2 p-3 flex justify-between rounded-xl'>
                    ...
                    <button className='text-red-600' onClick={() => removeCharge(e._id)}>Remove</button>
                  </li>
                ))}
             </ul>
