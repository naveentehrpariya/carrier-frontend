...
// Simple cache to store documents per employee
...
export default function EmployeeDocuments({ employee, onClose, classes, text, isOpen = false, entityType = 'employee' }) {
    ...
    const [loading, setLoading] = useState(false); // Start with false since we fetch on-demand
    ...
    const fetchDocuments = async (force = false) => { 
        if (!employee || !employee._id || !isOpen) return;
        ...
        try {
            const resp = await Api.get(`/user/employee/docs/${employee._id}`, {
    ...
    useEffect(() => {
        ...
        // Cleanup function to abort requests when modal closes or component unmounts
        return () => {
            if (abortController) {
                abortController.abort();
            }
                ...
                {file ? (
                    <div className='selectedMedia mb-6 relative'>
                        ...
                        {fileMime === 'doc' && (
                            <div className='w-full rounded-xl bg-gray-100 min-h-[300px] flex flex-col items-center justify-center'>
                                {!previewingUpload ? (
                                    ...
                                    <iframe className='w-full rounded-xl h-[300px]' src={URL.createObjectURL(file)}></iframe>
                                )}
                            </div>
                        )}
                        ...
                        <div className='flex justify-center'>
                            <div>
                                <button onClick={UploadMedia} className='mt-2 bg-main text-white px-4 py-2 rounded-xl'>Upload Document</button>
                    ...
                    <div className="relative w-full mb-10 bg-white bg-dark2 rounded-lg border border-dashed">
                        ...
                        <label htmlFor="file-upload" className="z-20 flex flex-col-reverse items-center justify-center w-full cursor-pointer p-8">
                    ...
                    {documents && documents.length > 0 ? (
                        <div className='grid grid-cols-2 gap-2'>
                            {documents.map((f, i) => {
                                ...
                                return (
                                    <div key={i} className='relative py-4 px-2 border border-gray-700 rounded-2xl text-center'>
    ...
    // If being used in controlled mode (new usage), render as modal
    if (isOpen) {
        return (
    ...
    // Legacy usage with Popup wrapper (for backwards compatibility)
    return (
